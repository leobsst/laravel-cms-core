...
>
    <div class="flex items-center justify-between px-4 py-2 w-full mb-5">
        <span class="text-xs text-medigrey dark:text-dark-primary-300">Mode sombre</span>
        <button
            type="button"
            x-bind:class="darkMode ? 'bg-blue-500' : 'bg-gray-200'"
            x-on:click="darkMode = !darkMode"
            class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer
            rounded-full border-2 border-transparent transition-colors duration-200
            ease-in-out focus:outline-none ring-0 {{ $attributes->has('shadow-button') ? 'shadow-lg' : null }}"
            role="switch"
            aria-checked="false"
        >
    ...
    <p class="text-center opacity-50 text-xs mb-2 text-black dark:text-white">
        {{Setting::get('website_name')}} <strong>v{{ env('APP_VERSION') }}</strong>
    </p>
</div>
